...
public interface StudentRepository extends JpaRepository<Student, Integer>{
    ...
    @Query(value = "select student.id, student.address, student.batch_id, student.first_name, student.last_name, student.middle_name, student.rfid, student.student_id from student left join sub_student on student.id=sub_student.student_id where student.batch_id=:batchid OR (sub_student.status=\"include\" AND sub_student.schedule=:schedid)", nativeQuery = true)
    List<Student> getStudentsFromThisSchedule(@Param("batchid") int batchid,@Param("schedid") int schedid);

    @Query(value = "select student.id, student.address, student.batch_id, student.first_name, student.last_name, student.middle_name, student.rfid, student.student_id from student join sub_student on student.id=sub_student.student_id where sub_student.schedule=:schedid AND (sub_student.status=\"exclude\" AND sub_student.schedule=:schedid)", nativeQuery = true)
